...
mysql --skip-ssl -h grainflow-database-7brbih -u cogsflow_user -pCogsFlow2026SecurePass -D cogsflow_db -e "SELECT 1;" 2>&1
if [ $? -ne 0 ]; then
...
table_count=$(mysql --skip-ssl -h grainflow-database-7brbih -u cogsflow_user -pCogsFlow2026SecurePass -D cogsflow_db -e "SHOW TABLES;" 2>/dev/null | wc -l)
...
mysql --skip-ssl -h grainflow-database-7brbih -u cogsflow_user -pCogsFlow2026SecurePass -D cogsflow_db -e "DESCRIBE users;" 2>&1
if [ $? -eq 0 ]; then
