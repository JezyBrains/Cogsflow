-- ============================================
-- FIX FOR EXISTING TABLES WITH FOREIGN KEYS
-- Run this to drop foreign keys first, then recreate
-- ============================================
...
-- Drop foreign keys from bag_inspections
ALTER TABLE `bag_inspections` DROP FOREIGN KEY IF EXISTS `fk_bag_inspections_dispatch`;
ALTER TABLE `bag_inspections` DROP FOREIGN KEY IF EXISTS `fk_bag_inspections_batch`;
ALTER TABLE `bag_inspections` DROP FOREIGN KEY IF EXISTS `fk_bag_inspections_user`;

-- Drop foreign keys from inspection_sessions
ALTER TABLE `inspection_sessions` DROP FOREIGN KEY IF EXISTS `fk_inspection_sessions_dispatch`;
ALTER TABLE `inspection_sessions` DROP FOREIGN KEY IF EXISTS `fk_inspection_sessions_batch`;
ALTER TABLE `inspection_sessions` DROP FOREIGN KEY IF EXISTS `fk_inspection_sessions_inspector`;
...
-- ============================================
-- STEP 3: Recreate tables (NO foreign keys)
-- ============================================
...
SELECT 'Checking bag_inspections...' AS Status;
DESC bag_inspections;

SELECT 'Checking inspection_sessions...' AS Status;
DESC inspection_sessions;

SELECT '✅ SUCCESS! Tables recreated without foreign keys!' AS Status;
SELECT 'You can now test the inspection page!' AS NextStep;
